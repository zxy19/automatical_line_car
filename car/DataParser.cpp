...
void data_parser::Parser::processHTTP(char c) {
    if (flags[DATA_PARSER_HTTP_FLAG_IN_BODY]) {
        ...
        if (bufferLen >= flags[DATA_PARSER_HTTP_FLAG_LENGTH]) {
            data = utils::fromBufferLen(buffer, bufferLen);
        ...
        if (c == ' ') {
            query = utils::fromBufferLen(buffer, bufferLen);
        } else if (c == '\n') {
...
void data_parser::Parser::processMultiByte(char c) {
    ...
    if (bufferLen == 3) {
        ok = true;
        result = data_parser::Result(Type::MULTI_BYTE, "mb", utils::fromBufferLen(buffer, 3));
    }
}
