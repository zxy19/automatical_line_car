...
void loop() {
    ...
    while (Serial1.available()) {
        fromSerial2.nextChar(Serial1.read());
        if (fromSerial2.isOK()) {
            String result = getResponseFor(fromSerial2.getResult(), Type::BINARY, &Serial1);
            if (result != "")
                Serial.print(result);
            fromSerial2.clear();
        }
    ...
    while (Serial.available()) {
        fromSUB_SERIAL.nextChar(Serial.read());
        if (fromSUB_SERIAL.isOK()) {
            String result = getResponseFor(fromSUB_SERIAL.getResult(), Type::BINARY, &Serial);
            if (result != "")
                Serial.print(result);
